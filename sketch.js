...
function draw() {
  ...
  for (let x = 0; x < topVideoGraphics.width; x += 20) {
    for (let y = 0; y < topVideoGraphics.height; y += 20) {
      ...
      topVideoGraphics.fill(col); // 設定方框顏色
      topVideoGraphics.noStroke();
      topVideoGraphics.rect(x + 1, y + 1, 18, 18); // 繪製方框，置於單位格內

      // 繪製中間的黑色圓
      topVideoGraphics.fill(0); // 設定圓形顏色為黑色
      topVideoGraphics.ellipse(x + 10, y + 10, 5, 5); // 繪製圓形，置於方框中心
    }
  }
